...
class LogsTable extends DefaultTable {
	...
	public function listItem($arrParam = null, $options = null){
        ...
        if($options['task'] == 'list-update-share') {
            $result	= $this->tableGateway->select(function (Select $select) use ($arrParam, $options){
                $paginator = $arrParam['paginator'];
                $ssFilter  = $arrParam['ssFilter'];
                $date       = new \ZendX\Functions\Date();
                ...
                if(!empty($ssFilter['contact_id'])) {
                    $select -> where -> equalTo('contact_id', $ssFilter['contact_id']);
                }
                $select -> where -> NEST
                    -> equalTo('action', 'Share từ data MKT')
                    ->OR
                    -> equalTo('action', 'Thêm mới')
                    -> UNNEST;

                $select -> where -> equalTo('title', 'Liên hệ');
//                echo "<pre>";
