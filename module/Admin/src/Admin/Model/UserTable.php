...
//                $select -> where -> equalTo('status', 1);
	...
	public function saveItem($arrParam = null, $options = null){
		...
		if($options['task'] == 'update-login') {
		    $id = $arrData['id'];
		    $item = $this->getItem(array('id' => $arrData['id']));


//		    $login_ip = explode(',', $item['login_ip']);
//		    $login_ip[] = $_SERVER['REMOTE_ADDR'];
//		    $login_ip = implode(',',array_filter($login_ip));

		    $uip = unserialize($item['login_ip']);
		    $uip[$_SERVER['REMOTE_ADDR']] = date('Y-m-d H:i:s');
		    $login_ip = serialize($uip);
...
//		        'login_ip'        => $_SERVER['REMOTE_ADDR'],
        ...
        if($options['task'] == 'update-flag') {
            $permission_code    = $arrParam['permission_code'];

            $sql_update = 'UPDATE '.TABLE_USER.' SET flag = 1 WHERE permission_ids like \'%'.$permission_code.'%\'';
            $this->tableGateway->getAdapter()->driver->getConnection()->execute($sql_update);
        }

        if($options['task'] == 'signout') {
