...
//    			    $select -> where -> equalTo(TABLE_CONTRACT .'.user_id', $ssFilter['filter_user']);
				...
				if(isset($ssFilter['filter_codes'])) {
    			    $select -> where -> in(TABLE_CONTRACT .'.code', $ssFilter['filter_codes']);
    			}
                ...
                if(isset($ssFilter['filter_status_lock']) && $ssFilter['filter_status_lock'] != '') {
                    $select -> where -> equalTo(TABLE_CONTRACT .'.lock', $ssFilter['filter_status_lock']);
                }
                ...
                if(!empty($ssFilter['filter_contract_type'])) {
                    $select -> where -> equalTo(TABLE_CONTRACT .'.production_type_id', $ssFilter['filter_contract_type']);
                }
	    ...
	    if($options['task'] == 'list-item-lost') {
	        $result	= $this->tableGateway->select(function (Select $select) use ($arrParam, $options){
                $ssFilter   = $arrParam['ssFilter'];
                $date       = new \ZendX\Functions\Date();
				$number     = new \ZendX\Functions\Number();
...
//                $select -> where -> isNull(TABLE_CONTACT .'.id');
//                $select -> where -> isNotNull(TABLE_CONTRACT .'.contact_id');
//                $select -> where -> notEqualTo(TABLE_CONTRACT .'.contact_id', '');

                $select -> where -> NEST
                    -> isNull(TABLE_CONTACT .'.id')
                    -> isNotNull(TABLE_CONTRACT .'.contact_id')
                    -> notEqualTo(TABLE_CONTRACT .'.contact_id', '')
                    ->OR
                    -> equalTo(TABLE_CONTRACT .'.delete', 1)
                    -> UNNEST;
	    ...
	    if($options['task'] == 'list-production-item') {
	        $result	= $this->tableGateway->select(function (Select $select) use ($arrParam, $options){
				...
				if(!empty($ssFilter['filter_codes'])) {
                    $select -> where -> in(TABLE_CONTRACT .'.code', $ssFilter['filter_codes']);
                }
	    ...
	    if($options['task'] == 'list-item-new') {
	        $result	= $this->tableGateway->select(function (Select $select) use ($arrParam, $options){
                ...
                $select -> where-> NEST
                    -> equalTo(TABLE_CONTRACT .'.production_department_type', '')
                    ->Or
                    -> isNull( 'production_department_type')
                    -> UNNEST;
	    ...
	    if($options['task'] == 'list-item-warehouse') {
	        $result	= $this->tableGateway->select(function (Select $select) use ($arrParam, $options){
                $ssFilter   = $arrParam['ssFilter'];
                $date       = new \ZendX\Functions\Date();
				$number     = new \ZendX\Functions\Number();
                ...
                $select -> where -> greaterThanOrEqualTo(TABLE_CONTRACT .'.total_number_product', 1);
                ...
                if(isset($ssFilter['filter_hidden'])) {
                    $select->where->equalTo(TABLE_CONTRACT . '.hidden', $ssFilter['filter_hidden']);
                }
...
//                    $select -> where -> equalTo(TABLE_CONTRACT .'.user_id', $ssFilter['filter_user']);
		...
		if($options['task'] == 'list-item-warehouse') {
			$result	= $this->tableGateway->select(function (Select $select) use ($arrParam, $options){
				...
				// Chỉ lấy ra danh sách những đơn hàng có trạng thái kế toán là đã nhận hoàn.
                $select -> where -> equalTo(TABLE_CONTRACT .'.status_acounting_id', STATUS_CONTRACT_ACOUNTING_RETURN);
                $select -> where -> greaterThanOrEqualTo(TABLE_CONTRACT .'.total_number_product', 1);
		...
		if($options['task'] == 'list-item-export') {
		    $result	= $this->tableGateway->select(function (Select $select) use ($arrParam, $options){
                $select -> join(TABLE_CONTACT, TABLE_CONTACT .'.id = '. TABLE_CONTRACT .'.contact_id',
                    array(
                        'contact_phone' => 'phone',
                ...
                $select -> order(array(TABLE_CONTRACT .'.date' => 'DESC', TABLE_CONTRACT .'.created' => 'DESC'));
        ...
        if($options['task'] == 'list-by-id') {
            $result	= $this->tableGateway->select(function (Select $select) use ($arrParam, $options){
    			if(!empty($arrParam['data']['ids'])) {
    			    $select->where->in(TABLE_CONTRACT .'.id', $arrParam['data']['ids']);
    			}
			});
        }
        
        // Trả về đơn hàng cuối cùng của khách hàng ở trạng thái chưa thành công (Trạng thái dục đơn)
        if($options['task'] == 'contract-coincider') {
            $result = $this->tableGateway->select(function (Select $select) use ($arrParam){
                $ssFilter  = $arrParam['ssFilter'];
                $date      = new \ZendX\Functions\Date();

                $select -> limit(1);
                $select -> order(array('date' => 'DESC', 'created' => 'DESC'));
				$select -> where -> lessThanOrEqualTo(TABLE_CONTRACT .'.date', $date->formatToSearch($ssFilter['date']));
				if (is_array($ssFilter['contact_id']))
                $select -> where ->in(TABLE_CONTRACT .'.contact_id', $ssFilter['contact_id']);
                else $select -> where ->equalTo(TABLE_CONTRACT .'.contact_id', $ssFilter['contact_id']);
                $select -> where -> NEST
                        -> notEqualTo(TABLE_CONTRACT .'.status_check_id', STATUS_CONTRACT_CHECK_SUCCESS)
                        ->OR
        ...
        if($options['task'] == 'list-item-update-cost') {
            $result	= $this->tableGateway->select(function (Select $select) use ($arrParam, $options){
                ...
                if(!empty($ssFilter['filter_branch_kov'])) {
                    $select -> where -> like(TABLE_CONTRACT .'.options', '%'.$ssFilter['filter_branch_kov'].'%');
                }
                ...
                $select -> where-> NEST
                    -> notEqualTo(TABLE_CONTRACT .'.production_department_type', STATUS_CONTRACT_PRODUCT_POST)
                    ->OR
                    -> isNull( TABLE_CONTRACT .'.production_department_type')
                    -> UNNEST;
            });
        }
	...
	public function saveItem($arrParam = null, $options = null){
		...
		if ($options['task'] == 'update-code') {
            ...
            if (strlen($index) <= 6) {
				$i = 8 - strlen($index);
				if ($result['status_guarantee_id'] == 1) {
					$data['code'] = substr_replace("BH-000000",$index, $i);
				} else {
				...
				if ($result['status_guarantee_id'] == 1) {
					$data['code'] = substr_replace("BH-000000",$index, 2);
				} else {
					if ($productionType[$result['production_type_id']]['alias'] == DON_TINH) {
		...
		if ($options['task'] == 'update-contract-succes') {
            ...
            $result = $this->getItem(array('id' => $id));
            if($result['status_check_id'] == STATUS_CONTRACT_CHECK_SUCCESS)
                $data['date_success'] = !empty($result['date_success']) ? $result['date_success'] : date('Y-m-d H:i:s');
            else
                $data['date_success'] = null;
        ...
        if ($options['task'] == 'update-refund-contract') {
            ...
            foreach($products as $key=>$product_info){
                $price_diff = ($arrData['contract_product']['price'][$key] - $product_info['price']);
                ...
                if(!empty($arrData['contract_product']['refund'][$key])){
                    if($arrData['contract_product']['refund'][$key] < $product_info['numbers']){
                        ...
                        $old_x['price'] = $arrData['contract_product']['price'][$key];
                        $old_x['listed_price'] *= $factor;
                        $old_x['capital_default'] *= $factor;
                        $old_x['vat'] *= $factor;

                        $old_x['sale_price']    = $old_x['sale_price'] - $price_diff;
                        $old_x['total']         = $old_x['total'] * $factor + ($old_x['numbers'] * $price_diff);

                        $old_x['sales_new']     = $old_x['sales_new'] > 0 ? $old_x['sales_new'] * $factor + ($old_x['numbers'] * $price_diff) : 0;
                        $old_x['sales_care']    = $old_x['sales_care'] > 0 ? $old_x['sales_care'] * $factor + ($old_x['numbers'] * $price_diff) : 0;
                        $old_x['sales_old']     = $old_x['sales_old'] > 0 ? $old_x['sales_old'] * $factor + ($old_x['numbers'] * $price_diff) : 0;
                        $old_x['sales_cross']   = $old_x['sales_cross'] > 0 ? $old_x['sales_cross'] * $factor + ($old_x['numbers'] * $price_diff) : 0;
                        ...
                        $new_x['price'] = $arrData['contract_product']['price'][$key];
                        $new_x['listed_price'] *= $factor2;
                        $new_x['capital_default'] *= $factor2;
                        $new_x['vat'] *= $factor2;

                        $new_x['sale_price']    = $new_x['sale_price'] - $price_diff;
                        $new_x['total']         = $new_x['total'] * $factor2 + ($new_x['numbers'] * $price_diff);

                        $new_x['sales_new']     = $new_x['sales_new'] > 0 ? $new_x['sales_new'] * $factor2 + ($new_x['numbers'] * $price_diff) : 0;
                        $new_x['sales_care']    = $new_x['sales_care'] > 0 ? $new_x['sales_care'] * $factor2 + ($new_x['numbers'] * $price_diff) : 0;
                        $new_x['sales_old']     = $new_x['sales_old'] > 0 ? $new_x['sales_old'] * $factor2 + ($new_x['numbers'] * $price_diff) : 0;
                        $new_x['sales_cross']   = $new_x['sales_cross'] > 0 ? $new_x['sales_cross'] * $factor2 + ($new_x['numbers'] * $price_diff) : 0;
                        ...
                        $new_x['price']     = $arrData['contract_product']['price'][$key];
                        $new_x['sale_price'] = $new_x['sale_price'] - $price_diff;
                        $new_x['total']     = $new_x['total'] + ($new_x['numbers'] * $price_diff);

                        $new_x['sales_new']     = $new_x['sales_new'] > 0 ? $new_x['sales_new'] + ($new_x['numbers'] * $price_diff) : 0;
                        $new_x['sales_care']    = $new_x['sales_care'] > 0 ? $new_x['sales_care'] + ($new_x['numbers'] * $price_diff) : 0;
                        $new_x['sales_old']     = $new_x['sales_old'] > 0 ? $new_x['sales_old'] + ($new_x['numbers'] * $price_diff) : 0;
                        $new_x['sales_cross']   = $new_x['sales_cross'] > 0 ? $new_x['sales_cross'] + ($new_x['numbers'] * $price_diff) : 0;
                        $products_new_x[] = $new_x;
                    }
                }
                else{
                    ...
                    $old_x['price']         = $arrData['contract_product']['price'][$key];
                    $old_x['sale_price']    = $old_x['sale_price'] - $price_diff;
                    $old_x['total']         = $old_x['total'] + ($old_x['numbers'] * $price_diff);

                    $old_x['sales_new']     = $old_x['sales_new'] > 0 ? $old_x['sales_new'] + ($old_x['numbers'] * $price_diff) : 0;
                    $old_x['sales_care']    = $old_x['sales_care'] > 0 ? $old_x['sales_care'] + ($old_x['numbers'] * $price_diff) : 0;
                    $old_x['sales_old']     = $old_x['sales_old'] > 0 ? $old_x['sales_old'] + ($old_x['numbers'] * $price_diff) : 0;
                    $old_x['sales_cross']   = $old_x['sales_cross'] > 0 ? $old_x['sales_cross'] + ($old_x['numbers'] * $price_diff) : 0;
...
//             cập nhật lại ngày thành công của đơn hàng đầu tiên
            ...
            $dataNew['price'] = $data_new['price_total'];
		...
		if ($options['task'] == 'update-reduce') {
            ...
            $options['note_accounting'] = $arrData['note_accounting'];
            if(!empty($options)){
...
//    		        'sale_branch_id'          => $this->userInfo->getUserInfo('sale_branch_id'),
        ...
        if($options['task'] == 'add-kov-item') {
            ...
            if(!empty($contact_id)) {
                ...
                for($i = 0; $i < count($contract_product['product_id']); $i++){
                    if(!empty($contract_product['product_id'][$i])) {
                        $capital_default = $number->formatToData($contract_product['capital_default'][$i]) * $number->formatToData($contract_product['numbers'][$i]);
                        if($product_type_contract[$arrData['production_type_id']] == 'Đơn Hà Nội'){
                            $capital_default = $capital_default + $number->formatToData($contract_product['fee'][$i]) * $number->formatToData($contract_product['numbers'][$i]);
                        }
                        if(!empty($contract_product['product_return_id'][$i])){
                            // Tính giá vốn của sản phẩm bán sẵn cùng chi nhánh thì giá vốn = 0, khách chi nhánh thì giá vốn = 1/2
                            $product_return = $this->getServiceLocator()->get('Admin\Model\ProductReturnTable')->getItem(array('id' => $contract_product['product_return_id'][$i]));
                            $capital_default = $sales_manager['sale_branch_id'] == $product_return['sale_branch_id'] ? 0 : $capital_default / 2;
                        }
                        $contract_options['product'][$i]['key_id']           = $gid->getId(); // Tạo mã đối chiếu trong trường hợp bán lại hàng có sẵn.
                        $contract_options['product'][$i]['product_return_id']= $contract_product['product_return_id'][$i]; // id sản phẩm trong kho hàng có sẵn
                        $contract_options['product'][$i]['product_id']       = $contract_product['product_id'][$i]; // sản phẩm
                        $contract_options['product'][$i]['product_name']     = $contract_product['product_name'][$i]; // tên xe - năm sản xuất
                        ...
                        $contract_options['product'][$i]['listed_price']     = $number->formatToData($contract_product['listed_price'][$i]) * $number->formatToData($contract_product['numbers'][$i]); // giá niêm yết
                        $contract_options['product'][$i]['capital_default']  = $capital_default; // giá vốn mặc định
                        $contract_options['product'][$i]['total']            = $number->formatToData($contract_product['total'][$i]); // tổng tiền (chính là cột thành tiền)
                        $contract_options['product'][$i]['type']             = $contract_product['type'][$i];
                        $contract_options['product'][$i]['sale_branch_id']   = $sales_manager['sale_branch_id'];
                        $contract_options['product'][$i]['full_name']        = $contract_product['full_name'][$i];
                        $contract_options['product'][$i]['code']             = $contract_product['code'][$i];
                        $contract_options['product'][$i]['branch_id']        = $contract_product['branch_id'][$i]; // chi nhánh kov
                        $contract_options['product'][$i]['cost']             = $contract_product['cost'][$i]; // giá vốn kov
                        $contract_options['product'][$i]['cost_new']         = $contract_product['cost_new'][$i]; // % tăng giá vốn so với giá gốc kov
                        $contract_options['product'][$i]['fee']              = $contract_product['fee'][$i]; // phụ phí sản phẩm
                        $contract_options['product'][$i]['is_gif']           = $contract_product['is_gif'][$i]; // có phải sản phầm này là sản phẩm khuyến mãi
                        $contract_options['product'][$i]['discount_value']   = $number->formatToData($contract_product['discount_value'][$i]); // giá trị giảm giá của sản phẩm
                        $contract_options['product'][$i]['unit_type']        = $number->formatToData($contract_product['unit_type'][$i]); // Kiểu giảm giá sản phẩm 2-%, 1-vnđ
                    }
                }

                // Trừ số lượng hàng có sẵn khi lên đơn
                foreach($contract_options['product'] as $cop){
                    if(!empty($cop['product_return_id'])){
                        $check_pro = $this->getServiceLocator()->get('Admin\Model\ProductReturnTable')->getItem(array('id' => $cop['product_return_id']));
                        if(!empty($check_pro)){
                            $this->getServiceLocator()->get('Admin\Model\ProductReturnTable')->saveItem(array('data' => ['quantity' => $check_pro['quantity'] - $cop['numbers']], 'item' => $check_pro), array('task' => 'edit-item'));
                        }
                    }
                }
                ...
                $data = array(
                    'id'                      => $id,
                    'date'                    => !empty($arrData['date']) ? $date->formatToData($arrData['date']) : date('Y-m-d'),
                    'price_total'             => $number->formatToData($arrData['total_contract']),
                    'price_paid'              => 0,
                    'price_accrued'           => 0,
                    'price_owed'              => $number->formatToData($arrData['total_contract']),
                    'price_surcharge'         => 0,
                    'kov_status'              => 1,
                    'price_deposits'          => $number->formatToData($price_deposits),
                    'contact_id'              => $contact_id,
                    ...
                    'total_contract_discount' => $number->formatToData($arrData['total_contract_discount']),
                    'marketer_id'             => $arrItem['marketer_id'],
                    'product_group_id'        => $arrItem['product_group_id'],
                    'status_id'               => DA_CHOT,
                ...
                if($number->formatToData($arrData['total_contract']) > 0) {
                    $data['price_paid']   = $number->formatToData($arrData['bill_paid_price']);
                    $data['price_owed']   = $number->formatToData($arrData['total_contract']) - $number->formatToData($arrData['bill_paid_price']);
                } else {
                    $data['price_paid']   = 0;
                    $data['price_owed']   = 0;
                }
		...
		if($options['task'] == 'edit-kov-item') {
            ...
            for($i = 0; $i < count($contract_product['product_id']); $i++){
                if(!empty($contract_product['product_id'][$i])) {
                    ...
                    if(!empty($contract_product['product_return_id'][$i])){
                        // Tính giá vốn của sản phẩm bán sẵn cùng chi nhánh thì giá vốn = 0, khách chi nhánh thì giá vốn = 1/2
                        $product_return = $this->getServiceLocator()->get('Admin\Model\ProductReturnTable')->getItem(array('id' => $contract_product['product_return_id'][$i]));
                        $capital_default = $arrItem['sale_branch_id'] == $product_return['sale_branch_id'] ? 0 : $capital_default / 2;
                    }
                    $contract_options['product'][$i]['key_id']           = $gid->getId(); // Tạo mã đối chiếu trong trường hợp bán lại hàng có sẵn.
                    $contract_options['product'][$i]['product_return_id']= $contract_product['product_return_id'][$i]; // id sản phẩm trong kho hàng có sẵn
                    $contract_options['product'][$i]['product_id']       = $contract_product['product_id'][$i]; // sản phẩm
                    ...
                    $contract_options['product'][$i]['total']            = $number->formatToData($contract_product['total'][$i]); // tổng tiền (chính là cột thành tiền)
                    $contract_options['product'][$i]['type']             = $contract_product['type'][$i];
                    $contract_options['product'][$i]['sale_branch_id']   = $contract_product['sale_branch_id'][$i];
                    $contract_options['product'][$i]['full_name']        = $contract_product['full_name'][$i];
                    $contract_options['product'][$i]['code']             = $contract_product['code'][$i];
                    $contract_options['product'][$i]['branch_id']        = $contract_product['branch_id'][$i]; // chi nhánh kov
                    $contract_options['product'][$i]['cost']             = $contract_product['cost'][$i]; // giá vốn kov
            ...
            // Cập nhật số lượng sản phẩm bán sẵn trong đơn hàng.
            $list_product_return_id_old = []; // Danh sách sản phẩm có sẵn cũ
            foreach(unserialize($arrItem['options'])['product'] as $return_old){
                if(!empty($return_old['product_return_id'])){
                    $list_product_return_id_old[$return_old['product_return_id']] = $return_old['numbers'];
                }
            }
            $list_product_return_id_new = []; // Danh sách sản phẩm có sẵn mới
            foreach($contract_options['product'] as $return_new){
                if(!empty($return_new['product_return_id'])){
                    $list_product_return_id_new[$return_new['product_return_id']] = $return_new['numbers'];
                }
            }
            // khi thay đổi số lượng hoặc xóa sản phẩm có sẵn trong đơn cũ
            foreach($list_product_return_id_old as $key => $quantity_old){
                if(array_key_exists($key, $list_product_return_id_new)){
                    $quantity_change = $list_product_return_id_new[$key] - $quantity_old;
                    if($quantity_change != 0){
                        $check_pro = $this->getServiceLocator()->get('Admin\Model\ProductReturnTable')->getItem(array('id' => $key));
                        if (!empty($check_pro)) {
                            $this->getServiceLocator()->get('Admin\Model\ProductReturnTable')->saveItem(array(
                                'data' => ['quantity' => $check_pro['quantity'] - $quantity_change],
                                'item' => $check_pro
                            ), array('task' => 'edit-item'));
                        }
                    }
                }
                else{
                    $quantity_change = 0 - $quantity_old;
                    if($quantity_change != 0){
                        $check_pro = $this->getServiceLocator()->get('Admin\Model\ProductReturnTable')->getItem(array('id' => $key));
                        if (!empty($check_pro)) {
                            $this->getServiceLocator()->get('Admin\Model\ProductReturnTable')->saveItem(array(
            ...
            // Khi thêm mới một sản phẩm có sẵn mới
            foreach($list_product_return_id_new as $key => $quantity_new){
                if(!array_key_exists($key, $list_product_return_id_old)){
                    $check_pro = $this->getServiceLocator()->get('Admin\Model\ProductReturnTable')->getItem(array('id' => $key));
                    if (!empty($check_pro)) {
                        $this->getServiceLocator()->get('Admin\Model\ProductReturnTable')->saveItem(array(
                            'data' => ['quantity' => $check_pro['quantity'] - $quantity_new],
                            'item' => $check_pro
                        ), array('task' => 'edit-item'));
                    }
                }
			...
			if(isset($arrData['total_contract'])) {
			    $data['price'] = $number->formatToNumber($arrData['total_contract']);
			}

			if(isset($arrData['total_contract'])) {
				$data['price_total'] = $number->formatToNumber($arrData['total_contract']);
				$data['price_owed'] = $data['price_total'] - $number->formatToNumber($arrItem['price_paid']);
			}
			...
			if(isset($arrData['total_contract_vat'])) {
			    $data['vat'] = $number->formatToNumber($arrData['total_contract_vat']);
			}

			if(isset($arrData['fee_other'])) {
			    $data['fee_other'] = $number->formatToNumber($arrData['fee_other']);
			}

			if(isset($arrData['total_contract_discount'])) {
			    $data['total_contract_discount'] = $number->formatToNumber($arrData['total_contract_discount']);
			}
		...
		if ($options['task'] == 'update-status') {
            ...
            // Cập nhật ngày thành công cho contact
            $this->saveItem(array('data' => $id), array('task' => 'update-contract-succes'));
		}
