...
class ApiController extends ActionController {
    ...
    public function selectAction() {
        ...
        if(!empty($this->_params['data']['term'])) {
            ...
            foreach ($results AS $result) {
                $pos = strpos(mb_strtolower($result['text']), mb_strtolower($this->_params['data']['term']));
                if($pos !== false) {
                    $termResults[] = $result;
                }
            }
    ...
    public function loadKovProductsAction() {
        ...
        if(!empty($this->_params['data']['categoriId']))
            $ssFilter['filter_categoryId'] = $this->_params['data']['categoriId'];
        if(!empty($this->_params['data']['filter_keyword']))
            $ssFilter['filter_keyword'] = $this->_params['data']['filter_keyword'];
        $product_params = array(
            'paginator' => array('itemCountPerPage'=> $itemPerpage, 'currentPageNumber' => $curentPage),
            'ssFilter'  => $ssFilter,
        );
        $this->_viewModel['kovProducts'] = $this->getServiceLocator()->get('Admin\Model\KovProductsTable')->listItem($product_params, array('task' => 'list-item'));
        $this->_viewModel['count'] = $this->getServiceLocator()->get('Admin\Model\KovProductsTable')->countItem($product_params, array('task' => 'list-item'));
    ...
    public function loadKovProductsReturnAction() {
        ...
        $this->_viewModel['kovProducts'] = $this->getServiceLocator()->get('Admin\Model\ProductReturnTable')->listItem($product_params, array('task' => 'list-item'));
        $this->_viewModel['count'] = $this->getServiceLocator()->get('Admin\Model\ProductReturnTable')->countItem($product_params, array('task' => 'list-item'));
        $this->_viewModel['itemPerpage'] = $itemPerpage;
        $this->_viewModel['curentPage']  = $curentPage;
    ...
    public function addProductToListAction() {
        $product = $this->getServiceLocator()->get('Admin\Model\KovProductBranchTable')->getItem(array('productId' => $this->_params['data']['id'], 'branchId' => $this->_params['data']['branch_id']));
...
//          $post_data_test = '{"Id":"84cfb14b-a7b9-4781-87cd-7842a150b87e","Attempt":1,"Notifications":[{"Action":"product.update.440946","Data":[{"__type":"KiotViet.OmniChannelCore.Api.Shared.Model.WebhookProductUpdateRes, KiotViet.OmniChannelCore.Api.Shared","Id":7217768,"RetailerId":440946,"Code":"quat500g","Name":"Sản phẩm Test CRM không bán","FullName":"Sản phẩm Test CRM không bán","CategoryId":193221,"CategoryName":"Hoa Quả, Trái Cây","AllowsSale":true,"Type":2,"HasVariants":false,"BasePrice":500,"Weight":500,"ConversionValue":1,"Description":"1","ModifiedDate":"2020-10-10T14:19:46.1330000+07:00","isActive":true,"IsRewardPoint":true,"Attributes":[],"Units":[],"Inventories":[{"ProductId":7217768,"ProductCode":"quat500g","ProductName":"Sản phẩm Test CRM không bán","BranchId":3134,"BranchName":"[ JP ] Nishi-Ikebukuro","Cost":280000,"OnHand":8,"Reserved":0,"MinQuantity":5,"MaxQuantity":10},{"ProductId":7217768,"ProductCode":"quat500g","ProductName":"Sản phẩm Test CRM không bán","BranchId":13083,"BranchName":"[ JP ] Saitama - Shiki","Cost":290000,"OnHand":22,"Reserved":3,"MinQuantity":0,"MaxQuantity":999999999},{"ProductId":7217768,"ProductCode":"quat500g","ProductName":"Sản phẩm Test CRM không bán","BranchId":44650,"BranchName":"VN - HÀ NỘI","Cost":300000,"OnHand":0,"Reserved":0,"MinQuantity":0,"MaxQuantity":999999999}],"PriceBooks":[],"Images":["https://cdn-images.kiotviet.vn/kinkin/7dcf7286e616435094635286816e29fe.png"]}]}]}';
