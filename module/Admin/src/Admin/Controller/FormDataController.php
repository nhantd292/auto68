            ...
            foreach ($arrData as $key => $data) {
                switch ($data['type']) {
                    ...
                    case 'data_source':
                        ...
                        $value = !empty($item[$data['field']]) ? $data['data_source'][$item[$data['field']]][$field] : '';
                        break;
        ...
        if($this->getRequest()->isPost()) {
            if(!empty($this->_params['data']['cid'])) {
                ...
                foreach ($data_update as $key => $value){
                    $report_item = $this->getServiceLocator()->get('Admin\Model\MarketingReportTable')->getItem(array('date' => $value['date'], 'marketer_id' => $value['marketer_id'], 'type' => 'mkt_report_day_hour'), array('task' => "marketer-date"));
                    if(!empty($report_item)){
                        ...
                        if(!empty($total_cp)){
                            $count_data = $this->getServiceLocator()->get('Admin\Model\FormDataTable')->countItem(array('marketer_id' => $value['marketer_id'] ,'date' => $value['date'],), array('task' => 'by-condition'));
