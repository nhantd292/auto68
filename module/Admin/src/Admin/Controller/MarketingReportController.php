...
class   MarketingReportController extends ActionController
{
    ...
    public function formAction()
    {
        ...
        $task    = 'add-item';
        $caption = 'Báo cáo marketing theo giờ - Thêm mới';
        $item    = array();
        if (!empty($this->params('id'))) {
            ...
            if (!empty($item)) {
                ...
                $myForm->bind($item);
                $task    = 'edit-item';
                $caption = 'Báo cáo marketing theo giờ - Sửa';
            }
        } else {
            $data_default = array(
                'date' => date('d/m/Y'),
    ...
    public function saveAjaxAction()
    {
        ...
        if(!empty($arrParams['data']['params']['total_cp'])) {
            ...
            $count_data = $this->getServiceLocator()->get('Admin\Model\FormDataTable')->countItem(array('marketer_id' => $arrParams['item']['marketer_id'] ,'date' => $arrParams['item']['date'],), array('task' => 'by-condition'));
            $cost_ads = (int)($total_cp / $count_data);

            $params_update = array(
                'cost_ads' => $cost_ads,
                'marketer_id' => $arrParams['item']['marketer_id'],
