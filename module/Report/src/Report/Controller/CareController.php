...
//                        }
    ...
    public function care12Action() {
        ...
        if($this->getRequest()->isPost()) {
            ...
            $sales_target = $this->getServiceLocator()->get('Admin\Model\TargetTable')->report(array('ssFilter' => $where_report), array('task' => 'list-item-type'));
            foreach ($sales_target as $key => $value){
                if(!empty($value['params']) && array_key_exists($value['user_id'], $data_report[$value['production_date']])){
                    $params = unserialize($value['params']);
                    $data_report[$value['production_date']][$value['user_id']]['target'] +=  str_replace(",","",$params['sales']);
                    $data_report['total']['total']['target'] +=  str_replace(",","",$params['sales']);
                    $data_report['total'][$value['user_id']]['target'] +=  str_replace(",","",$params['sales']);
                    $data_report[$value['production_date']]['total']['target'] +=  str_replace(",","",$params['sales']);
                }
            }
...
//                'filter_sales_status_id'    => 'yes',
            ...
            foreach ($contracts as $key => $value){
                // Nếu người lên đơn nằm trong danh sách nhân viên chăm sóc.
                if (array_key_exists($value['user_id'], $data_report[$value['production_date']])){
                    $data_report[$value['production_date']][$value['user_id']]['cod_total']    += $value['price_transport'];
                    $data_report['total']['total']['cod_total']    += $value['price_transport'];
                    $data_report['total'][$value['user_id']]['cod_total'] += $value['price_transport'];
                    $data_report[$value['production_date']]['total']['cod_total'] += $value['price_transport'];
                    ...
                    if($ssFilter->report['product_cat_id']){
                        if (!empty($value['options'])) {
                            ...
                            if (count($options['product'])) {
                                foreach ($options['product'] as $k => $v) {
                                    if($v['product_id'] == $ssFilter->report['product_cat_id']) {
                                        $data_report[$value['production_date']][$value['user_id']]['sales_total']    += $v['total'];
                                        $data_report['total']['total']['sales_total']    += $v['total'];
                                        $data_report['total'][$value['user_id']]['sales_total']    += $v['total'];
                                        $data_report[$value['production_date']]['total']['sales_total']    += $v['total'];

                                        // Sales - Hủy sales
                                        if ($value['status_id'] == HUY_SALES) {
                                            $data_report[$value['production_date']][$value['user_id']]['sales_cancel_sale'] += $v['total'];
                                            $data_report['total']['total']['sales_cancel_sale'] += $v['total'];
                                            $data_report['total'][$value['user_id']]['sales_cancel_sale'] += $v['total'];
                                            $data_report[$value['production_date']]['total']['sales_cancel_sale'] += $v['total'];
                                        }
                                        // Sản xuất - Hủy sản xuất
                                        if ($value['production_department_type'] == STATUS_CONTRACT_PRODUCT_CANCEL) {
                                            $data_report[$value['production_date']][$value['user_id']]['sales_cancel'] += $v['total'];
                                            $data_report['total']['total']['sales_cancel'] += $v['total'];
                                            $data_report['total'][$value['user_id']]['sales_cancel'] += $v['total'];
                                            $data_report[$value['production_date']]['total']['sales_cancel'] += $v['total'];
                                        }
                                        // SẢN XUẤT _ Hủy không gửi
                                        if ($value['production_department_type'] == STATUS_CONTRACT_PRODUCT_NOT_SEND) {
                                            $data_report[$value['production_date']][$value['user_id']]['sales_not_send'] += $v['total'];
                                            $data_report['total']['total']['sales_not_send'] += $v['total'];
                                            $data_report['total'][$value['user_id']]['sales_not_send'] += $v['total'];
                                            $data_report[$value['production_date']]['total']['sales_not_send'] += $v['total'];
                                        }
                                        // Dục đơn - Đang vận chuyển
                                        if ($value['status_check_id'] == STATUS_CONTRACT_CHECK_SENDING || $value['status_check_id'] == STATUS_CONTRACT_CHECK_POST) {
                                            $data_report[$value['production_date']][$value['user_id']]['sales_sending'] += $v['total'];
                                            $data_report['total']['total']['sales_sending'] += $v['total'];
                                            $data_report['total'][$value['user_id']]['sales_sending'] += $v['total'];
                                            $data_report[$value['production_date']]['total']['sales_sending'] += $v['total'];
                                        }
                                        // dục đơn - hoàn
                                        if ($value['status_check_id'] == STATUS_CONTRACT_CHECK_RETURN) {
                                            $data_report[$value['production_date']][$value['user_id']]['sales_return'] += $v['total'];
                                            $data_report['total']['total']['sales_return'] += $v['total'];
                                            $data_report['total'][$value['user_id']]['sales_return'] += $v['total'];
                                            $data_report[$value['production_date']]['total']['sales_return'] += $v['total'];
                                        }
                                        // dục đơn - Giữ lại bưu điện
                                        if ($value['status_check_id'] == STATUS_CONTRACT_CHECK_KEEP) {
                                            $data_report[$value['production_date']][$value['user_id']]['check_keep'] += $v['total'];
                                            $data_report['total']['total']['check_keep'] += $v['total'];
                                            $data_report['total'][$value['user_id']]['check_keep'] += $v['total'];
                                            $data_report[$value['production_date']]['total']['check_keep'] += $v['total'];
                                        }
                                        // Dục đơn - Thành công
                                        if ($value['status_check_id'] == STATUS_CONTRACT_CHECK_SUCCESS) {
                                            $data_report[$value['production_date']][$value['user_id']]['sales_success'] += $v['total'];
                                            $data_report['total']['total']['sales_success'] += $v['total'];
                                            $data_report['total'][$value['user_id']]['sales_success'] += $v['total'];
                                            $data_report[$value['production_date']]['total']['sales_success'] += $v['total'];
                                        }
                                        // Kế toán - Đã nhận tiền
                                        if ($value['status_acounting_id'] == STATUS_CONTRACT_ACOUNTING_MONEY) {
                                            $data_report[$value['production_date']][$value['user_id']]['sales_money'] += $v['total'];
                                            $data_report['total']['total']['sales_money'] += $v['total'];
                                            $data_report['total'][$value['user_id']]['sales_money'] += $v['total'];
                                            $data_report[$value['production_date']]['total']['sales_money'] += $v['total'];
                                        }

                                        // Thanh toán trước
                                        $data_report[$value['production_date']][$value['user_id']]['deposit'] += $v['price_deposits'];
                                        $data_report['total']['total']['deposit'] += $v['price_deposits'];
                                        $data_report['total'][$value['user_id']]['deposit'] += $v['price_deposits'];
                                        $data_report[$value['production_date']]['total']['deposit'] += $v['price_deposits'];

                                        // Giảm trừ doanh thu
                                        $data_report[$value['production_date']][$value['user_id']]['sales_refund'] += $v['price_reduce_sale'];
                                        $data_report['total']['total']['sales_refund'] += $v['price_reduce_sale'];
                                        $data_report['total'][$value['user_id']]['sales_refund'] += $v['price_reduce_sale'];
                                        $data_report[$value['production_date']]['total']['sales_refund'] += $v['price_reduce_sale'];

//                                        $data_report[$value['production_date']][$value['user_id']]['cost_capital'] += $v['total_production'];
//                                        $data_report['total'][$value['user_id']]['cost_capital'] += $v['total_production'];
//                                        $data_report['total']['total']['cost_capital'] += $v['total_production'];
//                                        $data_report[$value['production_date']]['total']['cost_capital'] += $v['total_production'];
                                        $data_report[$value['production_date']][$value['user_id']]['cost_capital'] += $v['capital_default'];
                                        $data_report['total'][$value['user_id']]['cost_capital'] += $v['capital_default'];
                                        $data_report['total']['total']['cost_capital'] += $v['capital_default'];
                                        $data_report[$value['production_date']]['total']['cost_capital'] += $v['capital_default'];

                                        $data_report[$value['production_date']][$value['user_id']]['discount']     += $v['sale_price'];
                                        $data_report['total']['total']['discount']     += $v['sale_price'];
                                        $data_report['total'][$value['user_id']]['discount']     += $v['sale_price'];
                                        $data_report[$value['production_date']]['total']['discount']     += $v['sale_price'];
                                    }
                                }
                            }
                        }
                        ...
                        $data_report[$value['production_date']][$value['user_id']]['sales_total'] += $value['price_total'] - $value['vat'];
                        $data_report['total']['total']['sales_total'] += $value['price_total'] - $value['vat'];
                        $data_report['total'][$value['user_id']]['sales_total'] += $value['price_total'] - $value['vat'];
                        $data_report[$value['production_date']]['total']['sales_total'] += $value['price_total'] - $value['vat'];

                        // Sales - Hủy sales
                        if ($value['status_id'] == HUY_SALES) {
                            $data_report[$value['production_date']][$value['user_id']]['sales_cancel_sale'] += $value['price_total'] - $value['vat'];
                            $data_report['total']['total']['sales_cancel_sale'] += $value['price_total'] - $value['vat'];
                            $data_report['total'][$value['user_id']]['sales_cancel_sale'] += $value['price_total'] - $value['vat'];
                            $data_report[$value['production_date']]['total']['sales_cancel_sale'] += $value['price_total'] - $value['vat'];
                        }
                        // Sản xuất - Hủy sản xuất
                        if ($value['production_department_type'] == STATUS_CONTRACT_PRODUCT_CANCEL) {
                            $data_report[$value['production_date']][$value['user_id']]['sales_cancel'] += $value['price_total'] - $value['vat'];
                            $data_report['total']['total']['sales_cancel'] += $value['price_total'] - $value['vat'];
                            $data_report['total'][$value['user_id']]['sales_cancel'] += $value['price_total'] - $value['vat'];
                            $data_report[$value['production_date']]['total']['sales_cancel'] += $value['price_total'] - $value['vat'];
                        }
                        // SẢN XUẤT _ Hủy không gửi
                        if ($value['production_department_type'] == STATUS_CONTRACT_PRODUCT_NOT_SEND) {
                            $data_report[$value['production_date']][$value['user_id']]['sales_not_send'] += $value['price_total'] - $value['vat'];
                            $data_report['total']['total']['sales_not_send'] += $value['price_total'] - $value['vat'];
                            $data_report['total'][$value['user_id']]['sales_not_send'] += $value['price_total'] - $value['vat'];
                            $data_report[$value['production_date']]['total']['sales_not_send'] += $value['price_total'] - $value['vat'];
                        }
                        // Dục đơn - Đang vận chuyển
                        if ($value['status_check_id'] == STATUS_CONTRACT_CHECK_SENDING || $value['status_check_id'] == STATUS_CONTRACT_CHECK_POST) {
                            $data_report[$value['production_date']][$value['user_id']]['sales_sending'] += $value['price_total'] - $value['vat'];
                            $data_report['total']['total']['sales_sending'] += $value['price_total'] - $value['vat'];
                            $data_report['total'][$value['user_id']]['sales_sending'] += $value['price_total'] - $value['vat'];
                            $data_report[$value['production_date']]['total']['sales_sending'] += $value['price_total'] - $value['vat'];
                        }
                        // Sản xuất - hoàn
                        if ($value['status_check_id'] == STATUS_CONTRACT_CHECK_RETURN) {
                            $data_report[$value['production_date']][$value['user_id']]['sales_return'] += $value['price_total'] - $value['vat'];
                            $data_report['total']['total']['sales_return'] += $value['price_total'] - $value['vat'];
                            $data_report['total'][$value['user_id']]['sales_return'] += $value['price_total'] - $value['vat'];
                            $data_report[$value['production_date']]['total']['sales_return'] += $value['price_total'] - $value['vat'];
                        }
                        // Sản xuất - Giữ lại bưu điện
                        if ($value['status_check_id'] == STATUS_CONTRACT_CHECK_KEEP) {
                            $data_report[$value['production_date']][$value['user_id']]['check_keep'] += $value['price_total'] - $value['vat'];
                            $data_report['total']['total']['check_keep'] += $value['price_total'] - $value['vat'];
                            $data_report['total'][$value['user_id']]['check_keep'] += $value['price_total'] - $value['vat'];
                            $data_report[$value['production_date']]['total']['check_keep'] += $value['price_total'] - $value['vat'];
                        }
                        // Dục đơn - Thành công
                        if ($value['status_check_id'] == STATUS_CONTRACT_CHECK_SUCCESS) {
                            $data_report[$value['production_date']][$value['user_id']]['sales_success'] += $value['price_total'] - $value['vat'];
                            $data_report['total']['total']['sales_success'] += $value['price_total'] - $value['vat'];
                            $data_report['total'][$value['user_id']]['sales_success'] += $value['price_total'] - $value['vat'];
                            $data_report[$value['production_date']]['total']['sales_success'] += $value['price_total'] - $value['vat'];
                        }
                        // Kế toán - Đã nhận tiền
//                        if ($value['status_acounting_id'] == STATUS_CONTRACT_ACOUNTING_MONEY) {
//                            $data_report[$value['production_date']][$value['user_id']]['sales_money'] += $value['price_total'] - $value['vat'];
//                            $data_report['total']['total']['sales_money'] += $value['price_total'] - $value['vat'];
//                            $data_report['total'][$value['user_id']]['sales_money'] += $value['price_total'] - $value['vat'];
//                            $data_report[$value['production_date']]['total']['sales_money'] += $value['price_total'] - $value['vat'];
//                        }
                        $price_paid = $value['price_paid'] < $value['price_total'] ? $value['price_paid'] : $value['price_total'];
                        $data_report[$value['production_date']][$value['user_id']]['sales_money'] += $price_paid;
                        $data_report['total']['total']['sales_money'] += $price_paid;
                        $data_report['total'][$value['user_id']]['sales_money'] += $price_paid;
                        $data_report[$value['production_date']]['total']['sales_money'] += $price_paid;

                        // Thanh toán trước
                        $data_report[$value['production_date']][$value['user_id']]['deposit'] += $value['price_deposits'];
                        $data_report['total']['total']['deposit'] += $value['price_deposits'];
                        $data_report['total'][$value['user_id']]['deposit'] += $value['price_deposits'];
                        $data_report[$value['production_date']]['total']['deposit'] += $value['price_deposits'];
                        // Giảm trừ doanh thu
                        $data_report[$value['production_date']][$value['user_id']]['sales_refund'] += $value['price_reduce_sale'];
                        $data_report['total']['total']['sales_refund'] += $value['price_reduce_sale'];
                        $data_report['total'][$value['user_id']]['sales_refund'] += $value['price_reduce_sale'];
                        $data_report[$value['production_date']]['total']['sales_refund'] += $value['price_reduce_sale'];
