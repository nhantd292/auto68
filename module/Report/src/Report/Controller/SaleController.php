            ...
            foreach ($contracts as $key => $value){
                ...
                if (array_key_exists($value['user_id'], $data_report)) {
                    ...
                    if ($ssFilter->report['product_cat_id']) {
                                ...
                                foreach ($options['product'] as $k => $v) {
                                    if ($v['product_id'] == $ssFilter->report['product_cat_id']) {
                                        ...
                                        if ($value['production_department_type'] == STATUS_CONTRACT_PRODUCT_CANCEL) {
                                            $data_report[$value['user_id']]['sales_cancel'] += $v['total'];
                                        ...
                                        if ($value['production_department_type'] == STATUS_CONTRACT_PRODUCT_NOT_SEND) {
                                            $data_report[$value['user_id']]['sales_not_send'] += $v['total'];
                                            ...
                                            if (empty($value['marketer_id'])) {
                                                ...
                                                if (!empty($value['contact_contract_time_success'])) {
                                                    ...
                                                    $day = date('Y-m-d H:i:s',
                                                        $day_begin + $value['time_sales'] * 3600);
                                        ...
                                        if ($value['status_acounting_id'] == STATUS_CONTRACT_ACOUNTING_MONEY) {
                                            $data_report[$value['user_id']]['sales_money'] += $v['total'];
                                        ...
                                        // Giảm giá
                                        $data_report[$value['user_id']]['discount'] += $v['sale_price'];
                        ...
                        // Sản xuất - Hủy sản xuất
                        if ($value['production_department_type'] == STATUS_CONTRACT_PRODUCT_CANCEL) {
                            $data_report[$value['user_id']]['sales_cancel'] += $value['price_total'] - $value['vat'];
                            $data_report['total']['sales_cancel'] += $value['price_total'] - $value['vat'];
                        }
                        // SẢN XUẤT - Hủy không gửi
                        if ($value['production_department_type'] == STATUS_CONTRACT_PRODUCT_NOT_SEND) {
                            $data_report[$value['user_id']]['sales_not_send'] += $value['price_total'] - $value['vat'];
                            $data_report['total']['sales_not_send'] += $value['price_total'] - $value['vat'];
                        }
                        ...
                        // Dục đơn - hoàn
                        if ($value['status_check_id'] == STATUS_CONTRACT_CHECK_KEEP) {
                            $data_report[$value['user_id']]['check_keep'] += $value['price_total'] - $value['vat'];
                            $data_report['total']['check_keep'] += $value['price_total'] - $value['vat'];
                        }
                        // Dục đơn - Thành công
                        $price_paid = $value['price_paid'] < $value['price_total'] ? $value['price_paid'] : $value['price_total'];
                        if ($value['status_check_id'] == STATUS_CONTRACT_CHECK_SUCCESS) {
                            if (empty($value['marketer_id'])) {
                                $data_report[$value['user_id']]['sales_success_add'] += $value['price_total'] - $value['vat'];
                                $data_report['total']['sales_success_add'] += $value['price_total'] - $value['vat'];
                            } else {
//                                if (!empty($value['contact_contract_time_success'])) {
                                    $day_begin = strtotime($value['contact_contract_time_success']);
                                    $day = date('Y-m-d H:i:s', $day_begin + $value['time_sales'] * 3600);
                                    if ($day > $value['created']) {
//                                        $data_report[$value['user_id']]['sales_success_new'] += $value['price_total'] - $value['vat'];
//                                        $data_report['total']['sales_success_new'] += $value['price_total'] - $value['vat'];
                                        $data_report[$value['user_id']]['sales_success_new'] += $price_paid;
                                        $data_report['total']['sales_success_new'] += $price_paid;
                                    } else {
//                                        $data_report[$value['user_id']]['sales_success_add'] += $value['price_total'] - $value['vat'];
//                                        $data_report['total']['sales_success_add'] += $value['price_total'] - $value['vat'];
                                        $data_report[$value['user_id']]['sales_success_add'] += $price_paid;
                                        $data_report['total']['sales_success_add'] += $price_paid;
                                    }
//                                }
                            }
//                            if (!empty($value['contact_contract_time_success'])) {
...
//                            }
                        ...
                        // Kế toán - Đã nhận tiền
//                        if ($value['status_acounting_id'] == STATUS_CONTRACT_ACOUNTING_MONEY) {
//                            $data_report[$value['user_id']]['sales_money'] += $value['price_total'] - $value['vat'];
//                            $data_report['total']['sales_money'] += $value['price_total'] - $value['vat'];
//                        }
                        $data_report[$value['user_id']]['sales_money'] += $price_paid;
                        $data_report['total']['sales_money'] += $price_paid;
            ...
            foreach ($data_report as $key => $value){
                ...
                $percent_return  = (($value['sales_total'] - $value['sales_cancel_sale'] - $value['sales_cancel']) > 0 ? round(($value['sales_return'] + $value['check_keep'] + $value['sales_refund'] + $value['sales_not_send']) / ($value['sales_total'] - $value['sales_cancel_sale'] - $value['sales_cancel']) * 100, 2) : 0);
//                $revenue         = $value['sales_money'] - $value['cost_capital'] - $value['cost_ads'] - $value['cod_total'];
            ...
            $xhtmlItems .= '<tr class="text-bold text-red">
        						...
        						<td class="mask_currency text-right">'.($data_report['total']['sales_success_new'] + $data_report['total']['sales_success_add']).'</td> <!--Mới + mua thêm-->
